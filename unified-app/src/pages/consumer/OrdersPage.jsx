      ...
      {trackedOrder && (
        <div className="mb-8 rounded-[2rem] bg-white dark:bg-slate-800 p-6 shadow-sm sm:p-8">
          ...
          <div className="mt-8 border-t border-slate-100 dark:border-slate-700 pt-8">
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Items in transit</h3>
                ...
                return (
                  <div key={idx} className="flex items-center gap-4 rounded-xl border border-slate-100 dark:border-slate-700 p-3 bg-slate-50/50 dark:bg-slate-900/50">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Qty: {item.quantity} • Rs. {item.price}/each</p>
                    </div>
                  </div>
                )
              })}
      ...
      <div className="space-y-5">
        {visibleOrders.map((order) => (
          <article key={order.id} className="rounded-[2rem] bg-white dark:bg-slate-800 p-6 shadow-sm">
            ...
            <div className="mt-5">
              <p className="text-sm font-semibold text-slate-900 dark:text-white mb-3">Items in this order</p>
              ...
              {order.total ? <p className="mt-5 text-sm font-semibold text-slate-900 dark:text-white">Total: Rs. {order.total}</p> : null}
